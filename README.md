...
- **专注计划**：定时自动进入 / 退出专注，支持跨天、不重复（执行一次即停用）、精确闹钟（开始前 15 秒提醒，点击弹出「立刻开始 / 终止」倒计时对话框，全屏 / 锁屏强制弹出）、与手动专注冲突时 5 分钟决策窗口
...
- **设置**：默认专注时长、专注结束通知、超级岛开关（内置焦点通知白名单解锁模块，需 LSPosed 启用并勾选系统界面作用域）、主题模式（浅色 / 深色 / 跟随系统）、开始前二次确认、计划开始前提醒秒数
...
| LSPosed（可选）                                    | 小米超级岛需启用霜息的内置白名单解锁模块（作用域勾选系统界面）   |
