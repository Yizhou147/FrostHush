...
[![Release](https://img.shields.io/github/v/release/Yizhou147/FrostHush?display_name=tag&style=flat)](https://github.com/Yizhou147/FrostHush/releases)
...
- **专注计划**：定时自动进入 / 退出专注，支持跨天、不重复（执行一次即停用）、精确闹钟（开始前 N 秒提醒，点击弹出「立刻开始 / 终止」倒计时对话框，全屏 / 锁屏强制弹出）、冲突预判与提示（手动开始普通专注若覆盖某启用计划则弹窗告知该计划本日不生效、次日恢复；两个启用计划时段重叠时计划页顶部红色横幅提示 + 蓝色高亮跳转，冲突计划到点自动跳过当日）；计划内可添加休息段（按具体时间分段）
...
- **开始专注**：分段编辑器（专注 / 休息段卡片 + 时长胶囊点按输入 + 预设快捷填充）→ 预计结束时刻 + 确认「不可中途退出」→ 立即暂停所选应用；默认专注 / 休息时长可在设置中调整
