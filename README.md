...
「霜息 FrostHush」将雹（[Hail](https://github.com/aistra0528/Hail)）的专注模式独立为专一应用，聚焦于一件事：**不可打断的专注会话**。雹本体不含专注模式，专注模式是在雹基础上额外添加的功能，霜息将其独立成完整的专注应用。
...
雹（[Hail](https://github.com/aistra0528/Hail)，GPL-3.0）本体不含专注模式；专注模式是在雹基础上额外添加的功能。本项目将该功能独立为完整的专注应用，不含冻结 / 停止等雹的本体功能，专注相关逻辑参考雹的自写实现。
